...
public class Parqueaderos {
    ...
    public void mostrarOcupacionPorTipoVehiculo(String tipoVehiculo) {
        for (Garaje garaje : listaGarajes) {
            int cantidad = garaje.contarVehiculosPorTipo(tipoVehiculo);
            System.out.println("Garaje en " + garaje.getDireccion() + 
                             ": " + cantidad + " " + tipoVehiculo + "(s)");
        }
    }

    public void mostrarRecaudos() {
        double recaudoTotal = 0;
        System.out.println("\n--- Recaudos de la Red de Parqueaderos ---");
        for (Garaje garaje : listaGarajes) {
            double recaudoGaraje = garaje.calcularIngresos();
            System.out.println("Garaje en " + garaje.getDireccion() + ": $" + recaudoGaraje);
